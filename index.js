...
fs.readdir("./commands/", (err, files) => {
    ...
    if (jsFiles.length <= 0) {
        console.log("Impossible de trouver les fichiers");
        return;
    }
    ...
    jsFiles.forEach((f, i) => {
        ...
        console.log(`Le ficher ${f} est OK !`);
...
client.on("guildMemberAdd", member => {
    ...
    var joinEmbed = new discord.MessageEmbed()
        ...
        .setDescription(`Hoi ${member.user.username}, **Bienvenue sur le serveur**`)
        .setColor("#00FF00")
        .setFooter("L'utilisateur a rejoint")
        .setTimestamp();
...
client.on("ready", async () => {
    ...
    let statuses = [
        "Regarde les étoiles",
        `En ligne sur ${client.guilds.cache.size}`,
        "joue avec les constelation"
    ]
